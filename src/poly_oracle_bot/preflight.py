...
async def run_preflight(cfg: AppConfig, db_path: Path, timeout_seconds: float = 10.0) -> list[CheckResult]:
    results: list[CheckResult] = [
        _path_writable("db_parent", db_path.parent),
        _path_writable("telemetry_parent", Path(cfg.telemetry.path).parent),
        _live_credentials_check(cfg),
        _telegram_check(cfg),
    ]
...
def _live_credentials_check(cfg: AppConfig) -> CheckResult:
    ...
    return CheckResult("live_credentials", True, "required variables present")
